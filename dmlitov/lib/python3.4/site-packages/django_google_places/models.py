...
class Place(models.Model):
    ...
    class QuerySet(_QuerySet):
        def get_or_create(self, **kwargs):
            if 'reference' in kwargs:
                kwargs.update(reference_sha1=Place.create_reference_sha1(kwargs.get('reference')))
                del kwargs['reference']
            return _QuerySet.get_or_create(self, **kwargs)

        def filter(self, reference=None, **kwargs):
            if 'reference' in kwargs:
                kwargs.update(reference_sha1=Place.create_reference_sha1(kwargs.get('reference')))
                del kwargs['reference']
            return _QuerySet.filter(self, **kwargs)

        def exclude(self, **kwargs):
            if 'reference' in kwargs:
                kwargs.update(reference_sha1=Place.create_reference_sha1(kwargs.get('reference')))
                del kwargs['reference']
            return _QuerySet.exclude(self, **kwargs)

        def reference(self, reference):
            return self.filter(reference_sha1=Place.create_reference_sha1(reference))
    ...
    def get_absolute_url(self):
        return reverse('django_google_places', pk=self.pk)
    ...
    def populate_from_api(self, place):
        """
        ...
        """
        if self.pk:
            # We're populating an existing Place, make sure folks know it's being updated...
        ...
        self.formatted_phone_number = place.details.get('formatted_phone_number')
        self.international_phone_number = place.details.get('international_phone_number')
...
class AddressComponent(models.Model):
    ...
    types = models.ManyToManyField(AddressComponentType, related_name='address_components')
...
class OpenPeriod(models.Model):
    ...
    def __unicode__(self):
        if self.open_day != self.close_day:
            return u'%s %s to %s %s' % (self.get_open_day_display(), 
                                        self.open_time,
                                        self.get_close_day_display(), 
                                        self.close_time)
        else:
            return u'%s %s to %s' % (self.get_open_day_display(),
                                     self.open_time,
                                     self.close_time)
...
class Review(models.Model):
    ...
    def __unicode__(self):
        return u'%s by %s' % (self.rating, self.author_name)
...
class ReviewAspect(models.Model):
    ...
    def __unicode__(self):
        return u'%s for %s' % (self.rating, self.get_aspect_type_display())
